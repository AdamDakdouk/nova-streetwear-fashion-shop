...
- React Router leaves scroll position alone across navigations, so a new page inherited the previous one's offset — opening a product from halfway down the grid landed the shopper at the product's reviews. It looked intermittent because the inherited offset then clamps to the incoming page's height, so where you ended up depended on how tall that page happened to be (measured: 2600 on the grid became 908 on the product page, that page's maximum).
- `ScrollManager` in `AppLayout` starts new navigations at the top and restores the previous offset on back/forward.
- **The position is snapshotted before navigation, not after.** The obvious implementation — read `window.scrollY` when the component notices the new location — records the *clamped* number, because the browser has already adjusted it by then (a first attempt stored 906 instead of 2600). Instead it snapshots on the interactions that precede a navigation: a capture-phase `click` listener (so it runs before any handler that navigates) and `popstate` (which fires while the outgoing page is still on screen), plus `pagehide` for tab close.
- Restoring re-applies over a few animation frames rather than once, since the page isn't always at its final height the instant it renders and the browser clamps anything past the current bottom. Without the retry a deep position silently lands short.
- It ignores navigations where only the query string changed. A category filter or a search is the same page repositioning itself, and `ProductListPage` already scrolls its section title into place — `AppLayout` sits above it, so its layout effect runs *after* the page's and would otherwise undo that work.
